...
class Game:
    ...
    def choose_action(self, d):
        if len(d.keys()) == 0:
            return None
        probs = np.array(list(d.keys()))
        beta = 1 # increase beta to decrease the randomness, or decrease beta to increase the randomness.
        mu = np.max(probs)
        probs -= mu
        probs = np.exp(beta * probs)
        probs /= np.sum(probs)
        # print(probs)
        # try:
        action_idx = np.random.choice(len(probs), size=None, p=probs)
        # except:
            # print(d)
        action_idx = list(d.keys())[action_idx]
        action = d[action_idx][1]
        print(action)
        if len(action) == 2:
